...
public class RateLimitingFilter2 implements GatewayFilter {
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		.map(list -> {
			...
			Mono<RedisClusterNode> clusterGetNodeForKey = reactiveClusterConnection.clusterGetNodeForKey(convertUTF8ToByteBuff(access_Key.toString()));
			...
			return clusterGetNodeForKey
							...
							return redisClient
									.connect(new RedisCodec<Pair<String, String>, Long>() {
										...
										public Long decodeValue(ByteBuffer bytes) {
											String rawBoolean = Unpooled.wrappedBuffer(bytes).toString(StandardCharsets.UTF_8);
											return Long.valueOf(rawBoolean);
										}
			
										@Override
										public ByteBuffer encodeKey(Pair<String, String> key) {
											String keyRaw = key.toString();
						...
						.map(future -> {
								return future.thenApply(conn -> {
									RedisReactiveCommands<Pair<String, String>, Long> reactive = conn.reactive();
									return reactive
											.multi()
											.flatMap(v -> {
						...
						.map(txnResult -> {
							return txnResult.<Mono<Boolean>>thenApply(results -> {
								return results.map(result -> {
												long consumed = ((List<Long>)result.get(1)).size();
